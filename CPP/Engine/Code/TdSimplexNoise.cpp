...
float tdOctaveNoise2D(const float octaves, const float persistence, const float scale, const float x, const float y, const int perm[])
{
	float total = 0;
...
// For each octave, a higher frequency/lower amplitude function will be added to the original.
// The higher the persistence [0-1], the more of each succeeding octave will be added.
float tdOctaveNoise3D(const float octaves, const float persistence, const float scale, const float x, const float y, const float z, const int perm[])
{
	float total = 0;
	float frequency = scale;
	float amplitude = 1;
...
// For each octave, a higher frequency/lower amplitude function will be added to the original.
// The higher the persistence [0-1], the more of each succeeding octave will be added.
float tdOctaveNoise4D(const float octaves, const float persistence, const float scale, const float x, const float y, const float z, const float w, const int perm[])
{
	float total = 0;
	float frequency = scale;
	float amplitude = 1;
...
float tdScaledOctaveNoise2D(const float octaves, const float persistence, const float scale, const float loBound, const float hiBound, const float x, const float y, const int perm[])
{
	return tdOctaveNoise2D(octaves, persistence, scale, x, y, perm) * (hiBound - loBound) / 2 + (hiBound + loBound) / 2;
}

// 3D Scaled Multi-octave Simplex noise.
//
// Returned value will be between loBound and hiBound.
float tdScaledOctaveNoise3D(const float octaves, const float persistence, const float scale, const float loBound, const float hiBound, const float x, const float y, const float z, const int perm[])
{
	return tdOctaveNoise3D(octaves, persistence, scale, x, y, z, perm) * (hiBound - loBound) / 2 + (hiBound + loBound) / 2;
}

// 4D Scaled Multi-octave Simplex noise.
//
// Returned value will be between loBound and hiBound.
float tdScaledOctaveNoise4D(const float octaves, const float persistence, const float scale, const float loBound, const float hiBound, const float x, const float y, const float z, const float w, const int perm[])
{
	return tdOctaveNoise4D(octaves, persistence, scale, x, y, z, w, perm) * (hiBound - loBound) / 2 + (hiBound + loBound) / 2;
}

// 2D Scaled Simplex raw noise.
//
// Returned value will be between loBound and hiBound.
float tdScaledRawNoise2D(const float loBound, const float hiBound, const float x, const float y, const int perm[])
{
	return tdRawNoise2D(x, y, perm) * (hiBound - loBound) / 2 + (hiBound + loBound) / 2;
}

// 3D Scaled Simplex raw noise.
//
// Returned value will be between loBound and hiBound.
float tdScaledRawNoise3D(const float loBound, const float hiBound, const float x, const float y, const float z, const int perm[])
{
	return tdRawNoise3D(x, y, z, perm) * (hiBound - loBound) / 2 + (hiBound + loBound) / 2;
}

// 4D Scaled Simplex raw noise.
//
// Returned value will be between loBound and hiBound.
float tdScaledRawNoise4D(const float loBound, const float hiBound, const float x, const float y, const float z, const float w, const int perm[])
{
	return tdRawNoise4D(x, y, z, w, perm) * (hiBound - loBound) / 2 + (hiBound + loBound) / 2;
}
...
float tdRawNoise3D(const float x, const float y, const float z, const int perm[])
{
...
float tdRawNoise4D(const float x, const float y, const float z, const float w, const int perm[])
{
