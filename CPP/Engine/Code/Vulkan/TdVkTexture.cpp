...
void tdVkTextureSetTestPixel(TdVkInstance* vulkan, TdVkTexture* texture, Vector2 pos, Color color)
{
	assert(vulkan);
	assert(texture);

	uint8 *color_data = (uint8*)tdVkTextureGetMapMemory(vulkan, texture);
	if (color_data)
	{
		int32 i = ((int)pos.x + (int)pos.y * texture->width) * 4;
		*color_data++ = (uint8)(color.r * 255);
		*color_data++ = (uint8)(color.g * 255);
		*color_data++ = (uint8)(color.b * 255);
		*color_data = (uint8)(color.a * 255);
		tdVkTextureSetMapMemory(vulkan, texture);
	}
}

void* tdVkTextureGetMapMemory(TdVkInstance* vulkan, TdVkTexture* texture)
{
	assert(vulkan);
	assert(texture);
	assert(texture->gpu_mem);

	void *cpu_mem;
	VkResult err = vkMapMemory(vulkan->device, texture->gpu_mem, 0, VK_WHOLE_SIZE, 0, &cpu_mem);
	if (err)
	{
...
void tdVkTextureSetMapMemory(TdVkInstance* vulkan, TdVkTexture* texture)
{
	assert(vulkan);
	assert(texture);
	assert(texture->gpu_mem);
	vkUnmapMemory(vulkan->device, texture->gpu_mem);
}

void tdVkCreateTexture(TdVkInstance* vulkan, TdPoint2 size, VkFormat format, TdVkTexture* texture)
{
	assert(vulkan);
	assert(texture);
	VkResult err;

	texture->width = (uint32)size.x;
	texture->height = (uint32)size.y;
	texture->mip_levels = 0;

	VkFormatProperties format_properties;
	vkGetPhysicalDeviceFormatProperties(vulkan->physical_device, format, &format_properties);

	VkBool32 use_staging = false;
	...
	texture->mem_size = mem_reqs.size;
